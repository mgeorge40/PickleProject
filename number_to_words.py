...
# Finds the longest English word that corresponds to the phone number or a subset of the phone number
...
def number_to_words(number):
    ...
    for l in range(length, 0, -1):
        ...
        for s in range(0, length-l+1, 1):
            word = getword(numbers[s:s+l], '', 0, wordlist)
...
def getword(numbers, output, count, wordlist):
    ...
    for n in letters: # cycles through each possible letter for the corresponding number
        ...
        if count+1 != len(numbers): # if we haven't reached the end of the numbers, loop again
            phone = getword(numbers, output, count+1, wordlist)
